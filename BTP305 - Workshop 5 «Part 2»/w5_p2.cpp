...
int main(int argc, char** argv)
{
	...
	auto updatePrice = [=](Book& lib)
	{
		double &price = lib.price();

		if ((lib.country().compare("US")) == 0)
			price *= usdToCadRate;
		else if ((lib.country().compare("UK")) == 0 && (lib.year() >= 1990 && lib.year() <= 1999))
			price *= gbpToCadRate;
	};
	...
	if (argc > 2)
	{
		...
		do
		{
			...
			if (f && noOfMovies < 5)
			{
				if (temp[0] != '#')
				{
					movies[noOfMovies] = sdds::Movie(temp);
					noOfMovies++;
				}
			}
		} while (f);
	...
	try {
		for (auto i = 0u; i < 10; ++i)
			std::cout << theCollection.operator[](i);
	}
	catch (exception& exp)
	{
